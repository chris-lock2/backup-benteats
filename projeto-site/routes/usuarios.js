...
var ClientRestaurant = require('../models').ClientRestaurant;
...
router.post('/login-suporte', function (req, res, next) {
	...
	let instrucaoSql = `select * from tb_user where emailSuporte='${email}' and senhaSuporte='${password}'`;
	console.log(instrucaoSql);
...
router.post('/register-contratante-validation', function (req, res, next) {
	...
	sequelize.query(instrucaoSql, {
		...
		if (resultado.length == 1) {
			res.status(409).send(`O E-mail "${email}" já existe em nosso banco`);
		} else {
			UsuariosContratante.create({
				...
				console.log("ERRO: Não foi possível fazer requisição com o servidor")
				console.error(erro);
				res.status(500).send(erro.message);
			});
		}
